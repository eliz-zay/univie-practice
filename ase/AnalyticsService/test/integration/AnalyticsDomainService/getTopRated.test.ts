...
describe('AnalyticsDomainService.getTopRated', () => {
  ...
  it('Successfully gets top rated events', async () => {
    ...
    const expectedResult = [
      ...
      {
        event: { id: 2, organizerId: 3, title: 'Event #2' },
        feedbackCount: 6,
        feedbackRating: 4,
        attendeeCount: 100
      },
      {
        event: { id: 5, organizerId: 4, title: 'Event #5' },
        feedbackCount: 8,
        feedbackRating: 4,
        attendeeCount: 50
      }
    ...
    const result = await service.getTopRated({ user: { id: 1 }, count: 3 });
  ...
  it('Throws error when RPC fails to return events', async () => {
    ...
    await expect(service.getTopRated({ user: { id: 1 }, count: 3 })).rejects.toEqual({ errorCode: 500, message: 'Internal error' })
  });
});
