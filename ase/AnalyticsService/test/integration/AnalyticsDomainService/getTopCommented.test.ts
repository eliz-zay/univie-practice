...
describe('AnalyticsDomainService.getTopCommented', () => {
  ...
  it('Successfully gets top commented events', async () => {
    ...
    const result = await service.getTopCommented({ user: { id: 1 }, count: 3 });
  ...
  it('Throws error when RPC fails to return events', async () => {
    ...
    await expect(service.getTopCommented({ user: { id: 1 }, count: 3 })).rejects.toEqual({ errorCode: 500, message: 'Internal error' })
  });
});
