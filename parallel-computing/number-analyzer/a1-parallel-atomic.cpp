...
void worker(
    ...
    double total_expected_sum = sum.load();
    while (!sum.compare_exchange_weak(total_expected_sum, total_expected_sum + worker_sum));
    ...
    {
        std::lock_guard<std::mutex> guard(number_counts_mutex);
        for(int i = 0; i < 10; i++) {
...
int main(int argc, char **argv) {
    ...
    std::atomic<int> primes{0}, nonprimes{0};
    std::atomic<int> consumed_count{0};
    std::atomic<double> mean{0.0}, sum = {0.0};
    std::vector<int> number_counts(10, 0); // vector for storing numbers ending with different digits (0-9)
